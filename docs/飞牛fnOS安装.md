...
飞牛版不接受 Docker 版的 `.bundle` 更新包。程序更新页会从 GitHub Release 检查最新版本、显示更新说明，并使用飞牛自己的应用数据目录管理 Git 代码。升级方式有两种：

- 飞牛应用中心提供新版本时直接升级。
- 首次安装或底层启动配置变化时，从 GitHub Release 下载 `.fpk` 并在应用中心手动覆盖安装。
- 日常版本在程序更新页直接从 GitHub 拉取代码，完成后自动重启，不会重新安装 FPK。
