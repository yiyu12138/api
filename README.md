...
| 飞牛 fnOS | `api-balance-vX.Y.Z.fpk` | 飞牛应用数据目录 | 检查版本并下载 FPK，再由应用中心确认升级 |
...
- Bark、Server酱、PushPlus、Telegram 和通用 Webhook 通知
- USD/CNY 自动汇率、配置导入导出、凭据加密存储
- 桌面、手机、飞牛桌面和 Android 共用响应式界面
- 亮色、暗色和跟随系统外观
- 免费版可添加 2 个站点，通过与安装编号绑定的离线激活码解锁完整功能

## 完整功能激活

每次安装会在本地生成独立安装编号。免费版最多添加 2 个站点；在“通用设置”中点击“联系购买”，邮件会自动带上安装编号。收到激活码后在同一页面粘贴并激活，即可解除站点数量限制。
...
- 飞牛：程序更新页显示 GitHub Release 说明并下载 `.fpk`，随后在应用中心确认覆盖升级。
